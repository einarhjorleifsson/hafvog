...
hv_import <- function(zipfiles, collapse_station = TRUE) {
  ...
  res <-
    list(leidangrar = purrr::map(res, "leidangrar") |> dplyr::bind_rows(.id = ".file"),
         ...
         merki = purrr::map(res, "merki") |> dplyr::bind_rows(.id = ".file"))
  ...
  res$merki <-      hv_order_merki(res$merki)
  ...
  if(nrow(res$merki) > 0) res$merki <- res$merki |> lh()
...
#' Order and type merki (fish tagging) columns
#'
#' @param d A tibble
#'
#' @return A tibble
hv_order_merki <- function(d) {
  tbl_colnames <- c(".file",
                    "synis_id", "utgafa", "audkenni", "numer", "fiskur_radnr")
  dummy <- readr::read_csv("\n", col_names = tbl_colnames, col_types = "ciicii")
  d <-
    dplyr::bind_rows(dummy, d)
  return(d)
}
